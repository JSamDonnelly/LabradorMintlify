...
<CardGroup cols={2}>
  <Card title="Keyword Search" icon="magnifying-glass">
    Type what you are looking for into the **Search criteria** box the way you would say it: a criterion name (`Non-text Content`), a number (`1.1.1`), or plain language such as `alt text`, `zoom`, or `autoplay`. The search reads each criterion's description and key points as well as its title, and a multi-word search matches criteria containing all of the words. The list updates as you type, with a note above the results naming the active search and a **Clear search** link beside it.
  </Card>
