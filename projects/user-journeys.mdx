...
When you tick **Test** on a manual step and save, Labrador mints a backing tracked item for that step inside the same save. The step keeps its name, note, and position, and it starts counting toward the journey's testing progress from that moment on. That item is listed on the project under **User journey steps**, a section that is collapsed by default and shows a count beside its heading.
...
- You can rename any step, including one that already has a tracked item behind it. Labrador renames that item to match, so the step and the item it points at never drift apart.
...
Deleting a journey removes the flow and its ordering. By default it leaves the steps alone: the pages, components, and flow states the steps pointed at remain in the project and keep every audit result, comment, and issue you logged against them. The confirmation dialog also offers to clear the steps out, so decide which you want before you confirm.
...
<Steps>
  ...
  <Step title="Decide what happens to the steps">
    **Also delete its steps** starts unticked. Leave it that way to keep every step, or tick it to clear the steps out. The dialog states the limit on what ticking it removes: "Only steps with no test results, and not used by another journey, are removed."
  </Step>
  <Step title="Confirm the impact">
    The journey is removed and you are returned to the project view. Steps that survive move into **Pages & Components**, with their test results untouched.
  </Step>
</Steps>
