...
This guide walks you through everything you need to run your first accessibility audit in Labrador. By the end, you'll have a live account, a project with at least one page, some criteria evaluated, and a report you can share, all without touching a spreadsheet. Labrador takes a manual hybrid approach, so you evaluate criteria yourself with automated axe scans available to clear the machine-detectable issues first. The whole process takes around ten minutes on a real site, or just a few minutes if you're exploring with a demo page.
...
<Steps>
  <Step title="Create your account">
    ...
    Once you're signed in, you land on the **Projects** dashboard. This is your home base for all audits. A ready-made demo project, **(Demo) University of Montauk**, is waiting there so you can explore a nearly completed audit before starting your own; see [Explore the demo project](#explore-the-demo-project) below.
  </Step>
...
Every new account comes with **(Demo) University of Montauk**, a sample audit that is most of the way through testing. It is the fastest way to see what a finished audit looks like before you run one yourself:

- Click around the completed pages to see documented issues, severities, and remediation guidance in place.
- Try **Export** and **View all issues** to see what your clients and developers would receive.
- Finish the last two pages yourself, or use the empty ones to try the axe scan and the batch N/A tool.
- Generate remediation guidance for an issue (the axe scan and remediation guidance both need the [Chrome extension](/integrations/chrome-extension)).

The demo never counts towards your plan's project limit, so it does not take the slot you need for real work, and you can delete it whenever you like.

### If you do not have a demo project

The demo is seeded automatically when an account is created, so accounts made before it existed never received one. If you have been using Labrador for a while and have never seen **(Demo) University of Montauk**, that is why, and nothing is wrong with your account.

To get it, look for the **Sample Project** button on the Projects dashboard, next to **\+ Create Project**. It appears on the Free plan whenever your account has no demo project, including after you delete one. On a paid plan the button is hidden, so [contact support](/help/contact-support) and we will add the demo to your workspace.
